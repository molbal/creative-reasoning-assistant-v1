...
BATCH_SIZE = 50  # Number of threads before updating the database
...
Author, that resulted in the continuation of the story. Only respond with the internal monologue please. Think step
by step within the monologue. Always keep in mind what happens in the Continuation and if you can, include dialogue
or event pieces from in it in the internal monologue, if it was an idea.
...
THOUGHT_PROCESS_TEMPLATE = """### System: You are a writer’s assistant.
...
def process_chunk(task, api_key, results):
    ...
    filled_prompt = PROMPT_TEMPLATE_TO_GENERATE_THOUGHT_PROCESS.format(prompt=prompt, response=response)
    thought_prompt = THOUGHT_PROCESS_TEMPLATE.format(context=prompt)
    final_response = call_openrouter(filled_prompt, api_key)

    if "Error:" not in final_response:
        final_response = f"<reasoning>{final_response}</reasoning><answer>{response}</answer>"
...
def update_database(results):
    ...
    with db_lock:
        c.executemany('''INSERT INTO training_data (source, type, subtype, prompt, response) 
                          VALUES (?, 'generative', 'thinking-autocomplete', ?, ?)''',
...
def main():
    ...
    conn = sqlite3.connect('text_chunks.db')
    c = conn.cursor()
    c.execute("SELECT id, source, prompt, response FROM training_data WHERE type='generative' and subtype='cc0-autocomplete' order by random() limit 1000")
